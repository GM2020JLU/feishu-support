-- Accept queued read-only reconciliation of dispatched field/transition writes.
CREATE TABLE project_activity_requests_next (
    ...
    kind TEXT NOT NULL CHECK(kind IN ('history','comments','relations','attachments','attachment_download','comment_reconcile','write_reconcile')),
...
INSERT INTO project_activity_requests_next(rowid,activity_id,bug_id,actor,grant_id,request_id,request_digest,reader_digest,kind,source_json,end_time_ms,state,attempt,lease_token,lease_expires_at,result_json,error_code,created_at,updated_at) SELECT rowid,activity_id,bug_id,actor,grant_id,request_id,request_digest,reader_digest,kind,source_json,end_time_ms,state,attempt,lease_token,lease_expires_at,result_json,error_code,created_at,updated_at FROM project_activity_requests;
