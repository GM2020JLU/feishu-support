ALTER TABLE project_activity_requests ADD COLUMN source_json TEXT CHECK(source_json IS NULL OR json_valid(source_json));
...
CREATE TABLE project_activity_requests_next (
    ...
    kind TEXT NOT NULL CHECK(kind IN ('history','comments','relations','attachments','attachment_download')),
